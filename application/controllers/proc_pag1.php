...
$query_car = "	SELECT 
					...
					TV.idTab_Produto
				FROM App_Produto  AP
					LEFT JOIN Tab_Valor AS TV ON AP.idTab_Produto = TV.idTab_Valor 
					LEFT JOIN Tab_Produto AS TP ON TV.idTab_Produto = TP.idTab_Produto
				WHERE 
...
while ($row_car = $resultado_car->fetch(PDO::FETCH_ASSOC)) {
    $DadosArray["itemId{$cont_item}"] = $row_car['idTab_Produto'];
    $DadosArray["itemDescription{$cont_item}"] = $row_car['Produtos'];
...
$cadastrar2=$pdo->prepare("update App_OrcaTrata set status=(:status), FormaPagamento=(:FormaPagamento), cod_trans=(:cod_trans) where idApp_OrcaTrata=(:reference)");
