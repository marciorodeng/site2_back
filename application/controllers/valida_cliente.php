		...
		if($count_celular == 0){
			...
			if($count_cliente == 0){	
				...
				if($count_cliente_senha == 0){	
					...
					$_SESSION['Site_Back']['id_Associado'.$idSis_Empresa] = $row_cliente_senha['idSis_Associado'];
					...
					if(isset($_SESSION['Site_Back']['id_Usuario'.$idSis_Empresa])){
						if($_SESSION['Site_Back']['id_Associado'.$idSis_Empresa] != $_SESSION['Site_Back']['id_Usuario'.$idSis_Empresa]){
							//Não faço nada
						}else{
					...
					if($_SESSION['Site_Back']['carrinho'.$idSis_Empresa]){
			...
			if($count_usuario_senha == 0){
				//Senha Associado correta? NÃO .
				$_SESSION['Site_Back']['msg'] = "Senha incorreta!";
				header("Location: login_cliente.php");
			
			} else {
				//Senha Associado correta? SIM. 
				//Pesquiso o usuário da plataforma na empresa em questão
				
				$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['idSis_Associado'] = $row_usuario_senha['idSis_Associado'];
				$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['Nome'] = $row_usuario_senha['Nome'];
				$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['Sexo'] = $row_usuario_senha['Sexo'];
				$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['Codigo'] = $row_usuario_senha['Codigo'];
				$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['Celular'] = $row_usuario_senha['CelularAssociado'];
				$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['Senha'] = $row_usuario_senha['Senha'];
				...
				$result_cliente = "SELECT *
									FROM 
										App_Cliente 
									WHERE
										idSis_Associado = '" .$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['idSis_Associado']. "' AND
										idSis_Empresa = '" .$idSis_Empresa. "'
									LIMIT 1";
				$resultado_cliente = mysqli_query($conn, $result_cliente);
				$row_cliente = mysqli_fetch_array($resultado_cliente, MYSQLI_ASSOC);
				...
				if($count_cliente == 0){
					...
					if($count_cliente_celular == 0){
						...
						$_SESSION['Site_Back']['msg'] = "Este Cliente já está cadastrado na plataforma Enkontraki,<br> mas ainda não está cadastrado na Empresa " . $row_empresa['NomeEmpresa'] . "!<br>Para inserí-lo nesta empresa, basta confirmar o cadastro!<br><br>Confirmar o Cadastro?";
						...
						$update_cliente = "UPDATE 
												App_Cliente 
											SET 
												idSis_Associado = '".$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['idSis_Associado']."',
												senha = '".$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['Senha']."',
												Codigo = '".$_SESSION['Site_Back']['Associado'.$idSis_Empresa]['Codigo']."'
											WHERE 
						...
						$_SESSION['Site_Back']['id_Associado'.$idSis_Empresa] = $row_cliente_celular['idSis_Associado'];
