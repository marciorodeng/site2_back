	...
	if(isset($_GET['tipo']) && isset($_GET['id']) ){	
		...
		if($_GET['tipo'] > '1'){
			...
			if(mysqli_num_rows($read_id_valor) > '0'){
				...
				if(mysqli_num_rows($read_produto) > '0'){
					foreach($read_produto as $read_produto_view){
						...
						if($_SESSION['Site_Back']['carrinho'.$_SESSION['Site_Back']['id_Cliente'.$idSis_Empresa]][$id_produto]){
							
							if($somar == '0'){
								
								unset($_SESSION['Site_Back']['carrinho'.$_SESSION['Site_Back']['id_Cliente'.$idSis_Empresa]][$id_produto]);
							
							}else if($somar == '2'){	
								
								$_SESSION['Site_Back']['carrinho'.$_SESSION['Site_Back']['id_Cliente'.$idSis_Empresa]][$id_produto] += 1;
			...
			if(mysqli_num_rows($read_id_valor) > '0'){
				...
				if($_SESSION['Site_Back']['carrinho'.$_SESSION['Site_Back']['id_Cliente'.$idSis_Empresa]][$_GET['id']]){	
					
					if($somar == '0'){
						
						unset($_SESSION['Site_Back']['carrinho'.$_SESSION['Site_Back']['id_Cliente'.$idSis_Empresa]][$_GET['id']]);
					
					}else if($somar == '2'){	
						
						$_SESSION['Site_Back']['carrinho'.$_SESSION['Site_Back']['id_Cliente'.$idSis_Empresa]][$_GET['id']] += 1;
