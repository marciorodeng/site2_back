	...
	if(in_array('',$dados)){
		...
		*/
		...
		if(($resultado_usuario) AND ($resultado_usuario->num_rows != 0)){
			//  Encontrou o Associado da empresa 5? SIM
			
			if(($resultado_cliente) AND ($resultado_cliente->num_rows != 0)){
				//  Encontrou o Cliente da empresa em questão? SIM
				$cadastrar = 1;
				//$erro = true;
				//$_SESSION['Site_Back']['msg'] = "Associado Sim, Cliente Sim";
			} else {
				// Encontrou o Cliente da empresa em questão? Não 
				$cadastrar = 2;
				//$erro = true;
				//$_SESSION['Site_Back']['msg'] = "Associado Sim, Cliente Não";
			}
		} else {
			// Encontoru o Associado da Empresa 5? Não
		...
		if($cadastrar == 1){
			...
			$_SESSION['Site_Back']['msg'] = "Cliente Já cadastrado.<br>Digite o Celular";
			$_SESSION['Site_Back']['msgcad'] = "Cliente Já cadastrado.<br>Digite o Celular";
			...
			$dados['senha'] = md5($dados['CelularCliente']);
			$CodInterno = md5(time() . rand());
			$DataCadastroCliente = date('Y-m-d', time());
			$Codigo = md5(time() . rand());
			...
			$_SESSION['Site_Back']['msg'] = "Cliente já cadastrado";
		}
	}	
}